    ...
    >
      ...
      <div className="w-full max-w-full overflow-x-hidden px-0.5">
        ...
        <div className="mt-6 w-full">
            ...
            <div className="grid gap-6 lg:grid-cols-3 animate-fade-in w-full">
              ...
              <div className="lg:col-span-2 card overflow-hidden flex flex-col w-full">
                ...
                {loading ? (
                  ...
                  <div className="p-12 text-center text-sm text-ink-400 w-full">
                  ...
                  <ul className="divide-y divide-ink-100 overflow-hidden w-full">
                    {orders.slice(0, 5).map(o => (
                      <li key={o.id} className={`flex items-center justify-between gap-4 px-4 sm:px-5 py-4 hover:bg-ink-50 border-l-4 ${statusColor[o.status] || 'border-transparent'}`}>
              ...
              <div className="space-y-4 w-full">
                ...
                <div className="card overflow-hidden w-full">
                  <div className="bg-gradient-to-br from-teal-600 to-emerald-500 px-5 py-6 text-center relative overflow-hidden">
                    ...
                    <p className="mt-3 font-bold text-white relative z-10 truncate">{user.name}</p>
                ...
                {/* Total dépensé */}
                <div className="card p-4 bg-gradient-to-r from-amber-50 to-orange-50 border-amber-100 w-full">
          ...
          {activeTab === 'orders' && (
            <div className="card max-w-4xl overflow-hidden animate-fade-in w-full">
              ...
              {orders.length === 0 ? (
                ...
                <ul className="divide-y divide-ink-100 w-full">
                  {orders.map(o => (
                    <li key={o.id} className={`flex flex-col sm:flex-row sm:items-center justify-between gap-4 px-4 sm:px-5 py-4 hover:bg-ink-50 border-l-4 ${statusColor[o.status] || 'border-transparent'}`}>
                      <div className="min-w-0 flex-1">
                        <p className="font-bold text-ink-900 truncate">{o.reference}</p>
          ...
          {activeTab === 'notifications' && (
            <div className="card max-w-2xl overflow-hidden animate-fade-in w-full">
              ...
              {notifications.length === 0 ? (
                ...
                <div className="divide-y divide-ink-100 w-full">
                  {notifications.map(n => (
                    <div key={n.id} className={`px-4 sm:px-5 py-4 flex gap-3 hover:bg-ink-50 ${!n.read_at ? 'bg-teal-50/50' : ''}`}>
                      <span className={`mt-2 h-2 w-2 rounded-full shrink-0 ${!n.read_at ? 'bg-teal-600' : 'bg-ink-300'}`} />
                      <div className="flex-1 min-w-0">
                        <p className={`text-sm ${!n.read_at ? 'font-semibold text-ink-900' : 'text-ink-700'} break-words`}>{n.title}</p>
                        <p className="text-xs text-ink-500 mt-0.5 break-words">{n.body}</p>
      ...
      {disputeModal.isOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 overflow-x-hidden overflow-y-auto">
          ...
          <div className="relative w-full max-w-md rounded-3xl bg-white shadow-2xl overflow-hidden max-h-[calc(100vh-2rem)] flex flex-col">
            <div className="bg-gradient-to-r from-red-600 to-rose-500 px-5 py-4 flex items-center gap-3 shrink-0">
              <div className="h-9 w-9 rounded-xl bg-white/20 flex items-center justify-center shrink-0">
                <AlertTriangle size={18} className="text-white" />
              </div>
              <div className="min-w-0 flex-1">
                <h3 className="font-bold text-white text-base truncate">Signaler un litige</h3>
                <p className="text-xs text-red-100 truncate">{disputeModal.orderRef}</p>
              </div>
              <button
                onClick={() => setDisputeModal({ isOpen: false, orderId: null, orderRef: '' })}
                className="h-8 w-8 grid place-items-center rounded-lg bg-white/20 hover:bg-white/30 text-white shrink-0"
              >
                <X size={16} />
              </button>
            </div>
            <form onSubmit={handleDisputeSubmit} className="p-5 space-y-4 overflow-y-auto flex-1">
              <div>
                <label className="block text-xs font-semibold text-ink-600 mb-1">Motif</label>
                <select className="w-full rounded-xl border border-ink-200 p-2.5 text-sm bg-white focus:outline-none focus:ring-2 focus:ring-red-500" value={disputeReason} onChange={e => setDisputeReason(e.target.value)} required>
              ...
              <div>
                ...
                <textarea className="w-full rounded-xl border border-ink-200 p-2.5 text-sm min-h-[100px] resize-none focus:outline-none focus:ring-2 focus:ring-red-500" value={disputeDesc} onChange={e => setDisputeDesc(e.target.value)} required />
              </div>
              <div className="flex gap-3 pt-2 shrink-0">
                <button type="button" onClick={() => setDisputeModal({ isOpen: false, orderId: null, orderRef: '' })} className="flex-1 py-2.5 rounded-xl border border-ink-200 hover:bg-ink-50 text-sm font-semibold transition-colors">Annuler</button>
                <button type="submit" disabled={disputeSubmitting} className="flex-1 py-2.5 rounded-xl bg-red-600 hover:bg-red-700 disabled:opacity-50 text-white text-sm font-semibold transition-colors">
